...
export default class Base {
    ...
    setConfig(config, isConstructing) {
        const
            ...
            pendingLazy    = me[lazyConfigsSymbol];
        ...
        for (key in config) {
            ...
            // Only skip lazy configs if we're being called from the constructor.
            if (!configDone[key] && !(isConstructing && configs[key]?.lazy)) {
                me[key] = config[key];
            }
        }
