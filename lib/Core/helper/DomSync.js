...
export default class DomSync {
        ...
        if (refOwner) {
            ...
            for (i = 0; i < affected.length; ++i) {
                ...
                if (checkEqualityOptions.refsFound.has(ref) || targetNode.retainElement) {
                    ...
                    // Safari and FF do not trigger a focusout event on node remove, so
                    // to keep focus tracking correct we have to force the issue.
                    targetNode.blur?.();
                    targetNode.remove();
    ...
    static syncChildren(options, targetElement) {
        ...
        // Prevents removing React content
