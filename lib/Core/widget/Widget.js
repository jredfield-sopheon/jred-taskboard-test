...
import Rectangle, { Point } from '../helper/util/Rectangle.js';
...
const
    ...
    isSingletonWidget       = w => !w.isRipple && !w.closest(w => w.isMessageDialog || w.isSharedTooltip);
...
export default class Widget extends Base.mixin(Localizable, Events, Delayable, Identifiable, Factoryable, KeyMap, RTL) {
    ...
    doDestroy() {
        ...
        if (eventRoot && !Object.values(Widget.identifiable.idMap).some(isSingletonWidget)) {
    ...
    alignTo(spec) {
        ...
        if (aligningToEvent) {
            ...
            // If we're aligning to an element which has layout
            if (hasLayout(target instanceof SVGElement ? target.closest('svg') : target)) {
                ...
                // space.
                spec.target = me.lastAlignSpec.targetRect =
                    spec.realignTarget || (spec.allowTargetOut ? Rectangle.from(target, positioned ? offsetParent : null, !positioned)
                        : DomHelper.isInView(target, false, me));
        ...
        // beforealign handlers may mutate the element, so take our measurements after
        const
        ...
        if (anchor?.edge) {
            ...
            // Jump up past inner elements which have focus or which the mouse is over because that may
            // cause us to read a transient background-color.
            if ((colourSource?.matches(':focus') || colourSource === GlobalEvents.currentOverElement) &&
        ...
        const
            ...
            dragListeners        = {
                ...
                dragover : event => {
                    ...
                    // Position using direct DOM access, do not go through the setXY method which clears
    ...
    whenVisible(callback, thisObj = this, args, id = callback.toString()) {
        ...
        if (me.isVisible && me.isPainted) {
            // The callback that is going to be called, must be deleted from `toCallWhenVisible` map (if existing)
            // in order to avoid calling it another time when queued calls are processed
            me.toCallWhenVisible?.delete(id);
    ...
    async show({ align, animate = true } = {}) {
        ...
        if (trigger) {
             ...
             * @typings source -> {Core.widget.Widget||any}
             */
    ...
    onElementResize(resizedElement, lastRect) {
        const
            me          = this,
            { element } = me,
            oldWidth    = me._width ?? me._oldWidth, // width gets nulled in changeWidth
